...
function createClassBreakInfos (breaks, classification, geomType) {
  ...
  return breaks.map((currBreak, index) => {
    const json = _.cloneDeep(classBreakInfos)
...
function createUniqueValueInfos (breaks, classificationDefinition, geomType) {
  const { colorRamp, baseSymbol } = setSymbology(breaks, classificationDefinition)

  // check that unique value fields are congruous
  if (classificationDefinition.uniqueValueFields.some(field => {
    // fail if any of the unique value field names are not found in the class break definition
    return !Object.keys(breaks[0]).includes(field)
  })) {
    throw new Error(
      'Unique value fields are incongruous: ' +
      Object.keys(breaks[0]) +
      classificationDefinition.uniqueValueFields)
  }

  return breaks.map((currBreak, index) => {
    const json = _.cloneDeep(uniqueValueInfos)
    json.value = parseUniqueValues(currBreak, classificationDefinition.fieldDelimiter)
